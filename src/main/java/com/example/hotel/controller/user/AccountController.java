...
public class AccountController {
    ...
    @GetMapping("/{id}/getCreditRecord")
    public ResponseVO getCreditRecord(@PathVariable int id) {
        return ResponseVO.buildSuccess(accountService.getCreditRecord(id));
    }

    @PostMapping("/rechargeCredit")
    public ResponseVO rechargeCredit(@RequestBody RechargeCreditVO rechargeCreditVO) {
        Integer userId = rechargeCreditVO.getUserId();
        Integer changeCredit = rechargeCreditVO.getChangeCredit();
        System.out.println(userId);
        System.out.println(changeCredit);
        return accountService.rechargeUserCredit(changeCredit, userId);
    }
}